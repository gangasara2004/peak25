    ...
    <div style={{ minHeight: 'calc(100vh - 57px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div className="card fade-up" style={{ maxWidth: 400, width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 22, marginBottom: 4 }}>Admin <span style={{ color: 'var(--yellow)' }}>Portal</span></h2>
          <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>PEAK '25 Meetup · Organizer Access</p>
        </div>
        {error && <div className="alert alert--error" style={{ marginBottom: 16 }}>{error}</div>}
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="field"><label>Email</label><input type="email" placeholder="admin@peak.lk" value={email} onChange={e => setEmail(e.target.value)} /></div>
...
const updateStatus = async (id: string, status: RegistrationStatus) => {
  ...
  const exportCSV = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'peak25-registrations.csv'; a.click()
  ...
  const filtered = useMemo(() => {
    ...
    return registrations.filter(r => {
      ...
      const matchSearch = !q || r.full_name.toLowerCase().includes(q) || r.email.toLowerCase().includes(q) || r.nic.toLowerCase().includes(q) || r.school.toLowerCase().includes(q)
  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 57px)', padding: '32px 20px 60px' }}>
      <div className="container--wide">
        ...
        {loading ? (
          ...
          <div className="card" style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>
            {search ? `No results for "${search}"` : 'No registrations found.'}
          </div>
        ) : (
          ...
          <div className="card fade-up" style={{ maxWidth: 560, width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            {selected.status !== 'pending' && (
              <button className="btn btn--secondary btn--full" style={{ marginTop: 10 }}
                onClick={() => updateStatus(selected.id, 'pending')} disabled={actionLoading}>
      ...
      {toast && (
        <div style={{ position: 'fixed', bottom: 24, right: 24, background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 20px', fontSize: 14, zIndex: 9999, color: 'var(--text)', boxShadow: '0 8px 32px rgba(0,0,0,0.4)', animation: 'fadeUp 0.3s ease' }}>
          {toast}
        </div>
      )}
    </div>
